---
title: "Tarea 4.3 – RNA-seq Differential Expression"
...
```{r visualizacion_inline, message=FALSE, warning=FALSE, fig.width=8, fig.height=4}
# Mostrar automáticamente las figuras ya generadas en results/diff_expr/
# (NO requiere el objeto "pseudocounts" en memoria)

library(knitr)

base_fig <- file.path("..", "results", "diff_expr")

figs_pdf <- c(
  file.path(base_fig, "pseudocounts", "pair_expression_culture.pdf"),
  file.path(base_fig, "pseudocounts", "pair_expression_genotype.pdf"),
  file.path(base_fig, "histograms",  "histograms_pvalue.pdf"),
  file.path(base_fig, "pvalue_fdr",  "pvalue_fdr.pdf")
)

# Chequeo rápido: si falta algún archivo, lo avisamos en el reporte
missing <- figs_pdf[!file.exists(figs_pdf)]
if (length(missing) > 0) {
  stop("Faltan estas figuras en results/diff_expr/: \n- ", paste(missing, collapse = "\n- "))
}

# Incluir las figuras (en PDF se insertan bien)
knitr::include_graphics(figs_pdf)
